...
export default function SwingCoach() {
  const { analyzing, results, analyze, reset, history } = useSwingAnalysis();
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="bg-gradient-to-br from-golf-800 to-golf-600 px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-6 rounded-b-3xl">
        ...
        <p className="text-golf-200 text-sm mt-1">
          {activeTab === 'history'
            ? 'Compare your previous swing analyses'
            : results
        ...
        {/* Tabs */}
        <div className="flex mt-4 bg-golf-900/30 rounded-xl p-1">
          <button
            onClick={() => setActiveTab('analyze')}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-semibold transition-colors ${
              activeTab === 'analyze'
                ? 'bg-white text-golf-800'
                : 'text-golf-200 hover:text-white'
            }`}
          >
            <Camera size={16} />
            Analyze
          ...
          >
            ...
            History
            {history.length > 0 && (
              <span className="bg-golf-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">
                {history.length}
              </span>
            )}
          </button>
